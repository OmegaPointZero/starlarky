...
def aes_unwrap_key_and_iv(kek, wrapped):
    ...
    for j in range(5,-1,-1): #counting down
        for i in range(n, 0, -1): #(n, n-1, ..., 1)
            ciphertext = bytearray(struct.pack('>Q', A^(n*j+i))) + bytearray(R[i])
            B = bytes(decrypt(ciphertext))
            A = struct.unpack('>Q', B[:8])[0]
            return (A, 0xa6a6a6a6a6a6a6a6) # return tuple w/ iv for integrity check
    '''
            R[i] = B[8:]
    return r"".join(R[1:]), A
    '''
...
def aes_unwrap_key(kek, wrapped, iv=0xa6a6a6a6a6a6a6a6):
    ...
    if key_iv != iv:
        fail("Integrity Check Failed: "+str(key_iv)+" (expected "+str(iv)+")")
    return key
...
def aes_wrap_key(kek, plaintext, iv=0xa6a6a6a6a6a6a6a6):
    ...
    for j in range(6):
        for i in range(1, n+1):
            ...
            R[i] = B[8:]
    return struct.pack('>Q', A) + r"".join(R[1:])

def aes_wrap_key_withpad(kek, plaintext):
    iv = 0xA65959A600000000 + len(plaintext)
    plaintext = plaintext + r"\0" * ((8 - len(plaintext)) % 8)
    if len(plaintext) == 8:
